...
// 1. Configuración Manual (IMPORTANTE: LLENAR ESTO EN GITHUB)
// Copia estos datos desde console.firebase.google.com -> Configuración del proyecto -> Tus apps
const MANUAL_CONFIG = {
apiKey: "AIzaSyBlk4PNyY9ogkCbfbDSO0I1YubtzegJnxA",
  authDomain: "iglesia-elim.firebaseapp.com",
  projectId: "iglesia-elim",
  storageBucket: "iglesia-elim.firebasestorage.app",
  messagingSenderId: "388995762714",
  appId: "1:388995762714:web:e7d39d3415a7fb59961fa3"
};

// 2. Lógica de Selección (No es necesario tocar esto)
// Esto permite que el código funcione tanto en el chat como en tu sitio real
let firebaseConfig;
...
export default function IglesiaElimApp() {
    ...
    const initAuth = async () => {
      try {
        if (typeof __initial_auth_token !== 'undefined' && __initial_auth_token) {
          ...
          // En Vercel usará esto. Asegúrate de tener habilitado "Anonymous Auth" en Firebase Console
          await signInAnonymously(auth);
        }
      } catch (err) {
        console.error("Error de autenticación:", err);
        if (err.code && err.code.includes('api-key')) {
            setError("Error: Falta poner la API KEY en el código de GitHub (src/App.jsx).");
        } else {
  ...
  useEffect(() => {
    ...
    // Lógica compatible para seleccionar la colección correcta
    let activitiesRef;
    if (COLLECTION_NAME.includes('/')) {
        // Modo Chat (Ruta larga)
        // Nota técnica: Firestore v9 modular prefiere no parsear slashes en collection()
        // Para compatibilidad simple, asumimos que si es chat usaremos la ruta explícita del entorno
        if (typeof __firebase_config !== 'undefined') {
  ...
  const generateSmartReport = () => {
    ...
    let report = `*INFORME MENSUAL - IGLESIA ELIM*\n🗓️ Mes: ${month.charAt(0).toUpperCase() + month.slice(1)}\n\n*RESUMEN DE LABORES:*\n`;
          ...
          <div className="mt-4 text-center">
            ...
            {!firebaseUser && MANUAL_CONFIG.apiKey.includes('TU_API_KEY') && typeof __firebase_config === 'undefined' && (
                <p className="text-amber-600 text-xs mt-2 font-bold bg-amber-50 p-2 rounded">
                  ⚠️ ATENCIÓN: Debes poner tus llaves de Firebase en el archivo src/App.jsx en GitHub
                </p>
            )}
          </div>
        </div>
